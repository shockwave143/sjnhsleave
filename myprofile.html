...
if (strlen($_SESSION['emplogin']) == 0) {
    ...
    if (isset($_POST['update'])) {
        ...
        $sql = "UPDATE tblemployees SET FirstName=:fname, Middle=:middle, LastName=:lname, Salary=:salary, Gender=:gender, Dob=:dob, Department=:department, Address=:address, City=:city, Country=:country, Phonenumber=:mobileno, Credits=:credits";
        ...
        $query->bindParam(':middle', $middle, PDO::PARAM_STR);
        ...
        if (isset($photoPath)) {
            $query->bindParam(':photoPath', $photoPath, PDO::PARAM_STR);
        }
    ...
    // Fetch employee record to retrieve photo path
    $fetchSql = "SELECT Photo FROM tblemployees WHERE EmailId=:eid";
    $fetchQuery = $dbh->prepare($fetchSql);
    $fetchQuery->bindParam(':eid', $eid, PDO::PARAM_STR);
    $fetchQuery->execute();
    $employee = $fetchQuery->fetch(PDO::FETCH_ASSOC);
...
<input id="middle" name="middle" value="<?php echo htmlentities($result->Middle);?>"  type="text" required>
</div>
...
<input id="position" name="" value="<?php echo htmlentities($result->Position);?>" type="text" autocomplete="off" readonly>
</div>
<div class="input-field col m6 s12">
<label for="salary">Salary</label>
<input id="salary" name="salary" value="<?php echo htmlentities($result->Salary);?>"  type="number" required>
</div>
...
<div>
<img src="<?php echo $photoPath; ?>" alt="Employee Photo" style="width: 200px; height: auto;">
</div>
<?php } ?>  
<br>
</div>  
